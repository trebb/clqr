...
\begin{LIST}{1cm}
  ...
  {
    Define symbol macro \retval{\VAR{foo}} which on evaluation
    evaluates expanded \VAR{form}.
  }
  
  \IT{(\SO*{MACROLET} (\OPn{(\VAR{foo} (\OPn{\VAR{macro-$\lambda$}})
      \orGOO{%
        \OPn{(\kwd{declare } \OPn{\NEV{\VAR{local-decl}}})}\\
        \NEV{\VAR{doc}}}{\}} \PROGN{\VAR{macro-form}})})
    \OPn{(\kwd{declare}
      \OPn{\NEV{\VAR{decl}}})} \PROGN{\VAR{form}})}
  {
    Evaluate \retval{\VAR{form}s} with locally defined mutually
    invisible macros \VAR{foo} which are enclosed in implicit
    \SO{block}s of the same name.
  }
    ...
    \VAR{function} \Op{\NEV{\VAR{doc}}})}
  {
    ...
    \VAR{function} applied to \VAR{place} and \VAR{arg}s will be
    stored into \VAR{place} and returned.
  }
