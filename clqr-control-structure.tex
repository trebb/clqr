...
\begin{LIST}{1cm}
  ...
  {
  Return \retval{values of \VAR{function}} called with
...
\begin{LIST}{1cm}
  ...
  {
  Return the \retval{values} of the first \OPn{\VAR{foo}} one of whose \VAR{key}s is
  \kwd{eql} \VAR{test}.
  Return \retval{values of \VAR{bar}s} if there is no matching \VAR{key}. 
  }
  ...
  \IT{(\xorGOO{\MC*{ECASE}\\
    ...
    \OPn{(\xorGOO{(\OPn{\NEV{\VAR{key}}})\\
        \NEV{\VAR{key}}}{\}} \PROGN{\VAR{foo}})})}
  {
  Return the \retval{values} of the first \OPn{\VAR{foo}} one of whose \VAR{key}s is \kwd{eql}
  \VAR{test}. Signal non-correctable/correctable \kwd{type-error} and
  return \retval{\NIL} if there is no matching \VAR{key}. 
  }
...
\begin{LIST}{1cm}
  ...
  {
  \EM{Simple Loop. } If \VAR{form}s do not contain any atomic Loop Facility
  ...
  \begin{LIST}{.5cm}
    ...
    \VAR{i-clause} and \VAR{j-clause}s; otherwise, evaluate \VAR{k-clause}
    and \VAR{l-clause}s.
    }
