...
\begin{LIST}{1cm}
  ...
  \IT{(\xorGOO{\MC*{DEFUN }\xorGOO{\VAR{foo}\\%FIXME:BLOCK
      (\kwd{setf} \VAR{ foo})}{.}\\ 
      \MC*{LAMBDA}}{\}}
  ...
  \VAR{form}s to \VAR{ord-$\lambda$}s. 
  }

  \IT{(\xorGOO{\SO*{FLET}\\
      \SO*{LABELS}}{\}} %FIXME:BLOCK
    (\OPn{(\VAR{foo}
      (\OPn{\VAR{ord-$\lambda$}}) 
...
\begin{LIST}{1cm}
  ...
  \IT{(\xorGOO{\MC*{DEFMACRO}\\%FIXME:BLOCK
  ...
  \IT{(\SO*{MACROLET} (\OPn{(\VAR{foo} (\OPn{\VAR{macro-$\lambda$}}) %FIXME:BLOCK
      \OPn{(\kwd{declare} \OPn{\NEV{\VAR{local-decl}}})} 
  ...
  \IT{(\MC*{DEFSETF} \NEV{\VAR{function}} \xorGOO{%  %FIXME:BLOCK
  ...
  }
  {
    ...
    into code setting the place accessed where \VAR{setf-$\lambda$}
    and \OPn{\VAR{s-var}} describe the arguments of \VAR{function} and
    the value(s) to be stored, respectively. \VAR{form}s must provide for returning
    the value(s) of \OPn{\VAR{s-var}}.
  }

  \IT{(\MC*{DEFINE-SETF-EXPANDER} \VAR{function}
    (\OPn{\VAR{macro-$\lambda$}}) %FIXME:BLOCK
    \OPn{(\kwd{declare} \OPn{\NEV{\VAR{decl}}})} \Op{\NEV{\VAR{doc}}}
    \PROGN{\VAR{form}})}
  {
  Specify how to \kwd{setf} a place accessed by
  \retval{\VAR{function}}. On invocation of (\kwd{setf} (\VAR{function}
    ...
    \FU{GET-SETF-EXPANSION}. In the expansion, the elements of macro
  ...
  \IT{(\FU*{GET-SETF-EXPANSION} \VAR{place} \Op{\VAR{environment}})}
  {
    ...
    and \retvaln{5}{get-form} specifying in terms of \VAR{arg-vars}
...
\begin{LIST}{1cm}
  ...
  \retval{\NIL} if all \VAR{test}s returns \NIL.
  }
  ...
  \VAR{test}.  Return \retval{\NIL} and signal correctable
  error/non-correctable \kwd{type-error}, respectively, if no element of
  \VAR{keys} matches.
  }
...
\begin{LIST}{1cm}
  ...
  \IT{(\MC*{DOTIMES} (\VAR{var} \VAR{integer} \Op{\VAR{result}}) %FIXME:BLOCK
    \OPn{(\kwd{declare} \OPn{\NEV{\VAR{decl}}})}
...
\begin{figure}%
  \begin{center}%
    \begin{sideways}%
      ...
      \left\{
      \begin{array}{l}
        \text{\LKWD{with} \VAR{var} \Op{\VAR{type}} \LKWD{=} \VAR{foo }} 
        \{\text{\LKWD{and} \VAR{var-n} \Op{\VAR{type}} \LKWD{=}
          \VAR{bar}\}}^{*} \\[.1ex]
        \left.\!
        \begin{array}{l}
          \text{\LKWD{for}}  \\
        ...
        \boxed{
          ...
          \begin{array}{l}
            ...
            \begin{array}{l}
              ...
              \text{\LKWD{it}}
            \end{array}
            \right\} \Op{\text{\LKWD{into }\VAR{var}}}\\
            \left.\!
            \begin{array}{l}
  ...
  \caption{Loop Facility, Overview.\label{loop-overview}}%
\end{figure}


\begin{LIST}{1cm}

  \IT{(\MC*{LOOP} \OPn{\VAR{form}})} %FIXME:BLOCK
  {
  Simple Loop. If \VAR{form}s do not contain any Loop Facility
  keywords, evaluate them forever. 
  }

  \IT{(\MC*{LOOP} \OPn{\VAR{form}})} %FIXME:BLOCK;TAGBODY
  {
  ...
  \begin{LIST}{.5cm}
    ...
    \IT{\Goo{\LKWD*{for}\XOR\LKWD*{as}\XOR\LKWD*{and}} \VAR{var} \Op{\VAR{type}}}
    {
    Begin of iteration control clauses; \VAR{var}s are initialized and
    stepped in parallel.
    }
    ...
    {
    Return immediately, skipping any \LKWD{finally} parts, with value of \VAR{form} or \LKWD{it}.
    }
    ...
    \IT{\LKWD*{loop-finish}}
    {
    Terminate \MC{loop} immediately returning any accumulated results.
    }
  \end{LIST}
\end{LIST}
