...
\begin{LIST}{1cm}
  ...
  {
  Make exported symbols of \VAR{other-packages} available in \VAR{package}, or remove them
  ...
  \IT{\V{\A package\A}\DF{\kwd{common-lisp-user}}\qquad\quad}
  ...
  \IT{(\FU*{PACKAGE-NAME} \VAR{package})\qquad\quad}
  {
  ...
  \IT{(\xorGOO{\FU*{INTERN}\\
      \FU*{FIND-SYMBOL}}{\}} \VAR{foo} \Op{\VAR{package}\DF{\V{\A package\A}}})}
  {
  ...
  \IT{(\xorGOO{%
      ...
      (\NEV{\VAR{var}}\text{ } \OP{\VAR{package}\DF{\V{\A package\A}}\text{ } \Op{\VAR{result}\DF{\NIL}}})\\
      \MC*{DO-ALL-SYMBOLS } (\VAR{var } \Op{\VAR{result}\DF{\NIL}})}{\}}
  ...
  \IT{(\FU*{REQUIRE} \VAR{module} \Op{\VAR{path-list}\DF{\NIL}})}
  {
  If not in \V{\A modules\A}, try paths in \VAR{path-list} to load
...
function (of \kwd{function}, macro, or special operator \VAR{name}).
...
\begin{LIST}{1cm}
  ...
  {
  Return fresh, uninterned symbol \retval{\kwd{\#:}\VAR{s}\VAR{n}} with \VAR{n} from
  \V{\A gensym-counter\A}. Increment \V{\A gensym-counter\A}. \index{*GENSYM-COUNTER*@\A GENSYM-COUNTER\A}
  }
  ...
  \IT{(\xorGOO{%
      \GFU*{DOCUMENTATION}\\
      (\kwd{SETF } \GFU{DOCUMENTATION})}{\}}
    \VAR{foo}
    \Goo{\kwd{'variable}%
      \XOR\kwd{'function}%
      \XOR\kwd{'compiler-macro}%
      \XOR\kwd{'method-combination}%
      \XOR\kwd{'structure}%
      \XOR\kwd{'type}%
      \XOR\kwd{'setf}%
      \XOR\T})}
  ...
  class except \kwd{t}; \V{\A terminal-io\A}.
  }

  \IT{\CNS*{NIL}\XOR{\CNS*{()}}}
  {
  Falsity; the empty list; the empty type, subtype of every type; \V{\A standard-input\A};
  \V{\A standard-output\A}; the global environment.
  }
