...
\begin{LIST}{1cm}
  ...
  \IT{(\FU*{PRINT-OBJECT} \VAR{object} \VAR{stream})} 
  ...
  \IT{(\FU*{PRINT-UNREADABLE-OBJECT} (\VAR{foo} \VAR{stream}
    ...
    \OPn{form})} 
  Print \VAR{foo} enclosed in \kwd{\#\boldmath$<$} and
  \kwd{\boldmath$>$}. Return \retval{\NIL}.
  ...
  \IT{(\FU*{WRITE-CHAR} \VAR{char} \Op{\VAR{stream}\DF{\T}}))}
  ...
  \IT{(\FU*{WRITE-SEQUENCE} \VAR{sequence}
    \VAR{stream}\orGOO{\kwd{:start}\VAR{ start}\DF{0}\\ 
      \kwd{:end} \VAR{ end}\DF{\NIL}}{\}})} 
...
\begin{LIST}{1cm}
  ...
  \IT{(\FU*{PARSE-NAMESTRING} \VAR{foo} \VAR{host}
    \VAR{default-pathname}\DF{\V{*default-pathname-defaults*}} 
