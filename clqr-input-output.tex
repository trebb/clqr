...
\begin{LIST}{1cm}
  ...
  \IT{(\FU*{READ-DELIMITED-LIST} \VAR{char}
    \OP{\VAR{stream}\DF{\V{*standard-input*}}
  ...
  \IT{(\FU*{READ-CHAR} \OP{\VAR{stream}\DF{\V{*standard-input*}}
  ...
  \IT{(\FU*{PEEK-CHAR}
    \OP{\VAR{mode}\DF{\NIL} \OP{\VAR{stream}\DF{\V{*standard-input*}} 
      \OP{\VAR{eof-error}\DF{\T} \Op{\VAR{eof-val}\DF{\NIL}
          \Op{\VAR{recursive}\DF{\NIL}}}}}})}
  {
  ...
  \IT{(\FU*{SET-SYNTAX-FROM-CHAR} \VAR{to-char} \VAR{from-char}
    \OP{\VAR{to-readtable}\DF{\V{*readtable*}}
...
\begin{LIST}{1cm}
  ...
  \IT{(\xorGOO{\FU*{PRIN1}\\ 
      ...
      \FU*{PPRINT}\\
      \FU*{PRINC}}{\}}
    \VAR{foo} \Op{\VAR{stream}\DF{\V{*standard-output*}}})}
  {
  ...
  \IT{(\FU*{TERPRI} \Op{\VAR{stream}\DF{\V{*standard-output*}}})}
  {
  ...
  \IT{(\xorGOO{\FU*{WRITE}\\
      ...
      \kwd{:stream} \VAR{ stream}\DF{\V{*standard-output*}}%
    }{\}})}
  {
  ...
  \IT{(\xorGOO{\FU*{PPRINT-FILL}\\
      \FU*{PPRINT-LINEAR}}{\}}
    \VAR{stream} \VAR{foo} \OP{\VAR{parenthesis}\DF{\T}
      \Op{\VAR{noop}}})}
  {
...
\begin{LIST}{1cm}
  ...
  \IT{(\FU*{CLOSE} \VAR{stream} \Op{\kwd{:abort}
      \VAR{bool}\DF{\NIL}})}
  {
