...
\begin{LIST}{1cm}
  ...
  \IT{(\xorGOO{\FU*{READ}\\
    ...
    \OP{\DES{\VAR{stream}}\DF{\V{*standard-input*}} \OP{\VAR{eof-err}\DF{\T}
  ...
  \IT{(\FU*{READ-DELIMITED-LIST} \VAR{char}
    \OP{\DES{\VAR{stream}}\DF{\V{*standard-input*}}
  ...
  \IT{(\FU*{READ-CHAR} \OP{\DES{\VAR{stream}}\DF{\V{*standard-input*}}
  ...
  \IT{(\FU*{PEEK-CHAR}
    \OP{\VAR{mode}\DF{\NIL} \OP{\DES{\VAR{stream}}\DF{\V{*standard-input*}} 
      \OP{\VAR{eof-error}\DF{\T} \Op{\VAR{eof-val}\DF{\NIL}
          \Op{\VAR{recursive}\DF{\NIL}}}}}})}
  {
  ...
  \IT{(\FU*{READ-LINE} \OP{\DES{\VAR{stream}}\DF{\V{*standard-input*}}
      \OP{\VAR{eof-err}\DF{\T} \Op{\VAR{eof-val}\DF{\NIL}
          \Op{\VAR{recursive}\DF{\NIL}}}}})}
  {
  ...
  {
  \retval{Case sensitivity attribute} (one of \kwd{:upcase}, \kwd{:downcase}, \kwd{:preserve}, \kwd{:invert}) of \VAR{readtable}. \kwd{setf}able.
  }

  \IT{(\FU*{COPY-READTABLE} \OP{\VAR{from-readtable}\DF{\V{*readtable*}}
  ...
  \IT{(\FU*{SET-SYNTAX-FROM-CHAR} \VAR{to-char} \VAR{from-char}
    \OP{\DES{\VAR{to-readtable}}\DF{\V{*readtable*}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection[Macro Chars]{Macro Characters}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{LIST}{1cm}
  ...
  {
  \index{\#R}%
  Binary, octal or hexadecimal number, or number of radix \VAR{n}. 
  }

  \IT{\KWD*{\#C(}\VAR{a b}\kwd{)}}
  ...
  \IT{\KWD*{\#+}\VAR{feature-expression} \VAR{form}}
  {
  Means \VAR{form} if \VAR{feature-expression} returns \T, means whitespace
  otherwise. 
  }

  \IT{\KWD*{\#--}\VAR{feature-expression} \VAR{form}}
  {
  Means whitespace if \VAR{feature-expression} returns \T, means \VAR{form} 
  otherwise. 
  }
...
\begin{LIST}{1cm}
  ...
  \IT{(\xorGOO{\FU*{PRIN1}\\ 
      ...
      \FU*{PPRINT}\\
      \FU*{PRINC}}{\}}
    \VAR{foo} \Op{\DES{\VAR{stream}}\DF{\V{*standard-output*}}})}
  {
  ...
  \IT{(\FU*{TERPRI} \Op{\DES{\VAR{stream}}\DF{\V{*standard-output*}}})}
  {
  ...
  \IT{(\xorGOO{\FU*{WRITE-STRING}\\
    ...
    \OP{\DES{\VAR{stream}}\DF{\V{*standard-output*}}}
    \orGOO{\kwd{:start} \VAR{ start}\DF{\LIT{0}}\\\kwd{:end} \VAR{
        end}\DF{\NIL}}{\}})}
  {
  ...
  \IT{(\xorGOO{\FU*{WRITE}\\
      ...
      \kwd{:stream } \DES{\VAR{stream}}\DF{\V{*standard-output*}}%
    }{\}})}
  {
  ...
  \IT{(\xorGOO{\FU*{PPRINT-FILL}\\
      \FU*{PPRINT-LINEAR}}{\}}
    \DES{\VAR{stream}} \VAR{foo} \OP{\VAR{parenthesis}\DF{\T}
      \Op{\VAR{noop}}})}
  {
  ...
  \IT{(\FU*{PPRINT-TABULAR} \DES{\VAR{stream}} \VAR{foo}
  ...
  \IT{(\FU*{SET-PPRINT-DISPATCH} \VAR{type} \VAR{function}
    \OP{\VAR{priority}\DF{\LIT{0}}
      \Op{\VAR{table}\DF{\V{*print-pprint-dispatch*}}}})}
  {
...
\begin{LIST}{1cm}
  ...
  \begin{LIST}{.5cm}
    ...
    {\index{\TLD D}\index{\TLD B}\index{\TLD O}\index{\TLD X}
    Print integer argument as number (decimal, binary, octal, or
    hexadecimal, respectively). With \kwd{:} group digits 
    \VAR{comma-interval} each; with \kwd{@}, always prepend a sign.
  }

    \IT{\KWD{\TLD}\OP{\VAR{width} \OP{\KWD{,}\Op{\VAR{dec-digits}} \OP{\KWD{,}\Op{\VAR{shift}\DF{\LIT{0}}}
    ...
    do the same after backing up one argument; if argument \kwd{eql} \LIT{1}
    ...
    \kwd{@}, take all remaining arguments. With \kwd{:}, argument should be
    a list of sublists. At each iteration step the elements of a new
    sublist are used by \VAR{text}. 
  }
    ...
    \IT{\KWD{\TLD}\OP{\VAR{prefix}\Goos{\kwd{,} \VAR{prefix}}}\Op{\kwd{:}}\Op{\kwd{@}}\KWD{/}\VAR{function}\KWD{/}}
    {\index{\TLD//}
    Call \VAR{function} with the arguments stream, format-ar\-gu\-ment,
    bool-colon, bool-at and \VAR{prefix}es for printing format-argument.
  }
...
\begin{LIST}{1cm}
  ...
  {
  Return \retval{\kwd{concatenated-stream}}.
  }
  ...
  {
  Return \retval{\kwd{broadcast-stream}}.
  }
  ...
  {
  Place \retval{\kwd{echo-stream}} between \VAR{input-stream} and
  \VAR{output-stream}. 
  }
  ...
  \IT{(\FU*{MAKE-TWO-WAY-STREAM} \VAR{input-stream}
    \VAR{output-stream})}
  {Return \retval{\kwd{two-way-stream}}.
  }
  ...
  \IT{(\FU*{LISTEN} \Op{\VAR{stream}\DF{\V{*standard-input*}}})}
  {
  ...
  stream. Append output to \VAR{string} and return \retval{values of
    \VAR{form}s} if \VAR{string} is given. Return \retval{string
