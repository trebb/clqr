...
\begin{LIST}{1cm}
  ...
  \IT{\KWD{\#}\VAR{n}\KWD{A}\OPn{\VAR{sequence}}}
  {
  \index{\#A}%
  ...
  \IT{\KWD*{\#--}\VAR{feature-expression} \VAR{form}}
  {
  Means \VAR{form} if \VAR{feature-expression} returns \NIL, means whitespace
  otherwise. 
  }
...
\begin{LIST}{1cm}
  ...
  \IT{(\FU*{PRINT-UNREADABLE-OBJECT} (\VAR{foo} \DES{\VAR{stream}}
    \Op{\kwd{:type} \VAR{bool}} \Op{\kwd{:identity} \VAR{bool}})
    \OPn{\VAR{form}})}
  {
  Enclosed in \kwd{\#\boldmath$<$} and \kwd{\boldmath$>$}, print
  \VAR{foo} by means of \VAR{form}s. Return \retval{\NIL}. 
  }
...
\begin{LIST}{1cm}
  ...
  \begin{LIST}{.5cm}
    ...
    \IT{\KWD{\TLD\Op{:}\Op{@}/}\VAR{function}\KWD{/}}
    {
    Call \VAR{function} with arguments stream, format-ar\-gu\-ment,
...
\begin{LIST}{1cm}
  ...
  {
  Return a \retval{string stream} accepting characters.
  }
