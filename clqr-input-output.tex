...
\begin{LIST}{1cm}
  ...
  \IT{\KWD{\TLD}\OP{\VAR{dec-digits}\DF{\LIT{2}}
    ...
    \Op{\KWD{:}}\Op{\kwd{@}}\KWD{\$}}
  {\index{\~\$@$\sim$\$}
  Print argument as fixes-format floating-point number. With \KWD{:},
  ...
  \IT{\KWD{\TLD}\Op{\KWD{:}}\Op{\KWD{@}}\KWD{\boldmath{$<$}}
    \OP{\Goo{\VAR{prefix}\DF{\NIL}\KWD{\TLD;}}\XOR\Goo{\VAR{per-line-prefix}\KWD{\TLD@;}}}
    \VAR{body}\OP{\KWD{\TLD;}\VAR{suffix}\DF{\NIL}}\KWD{\TLD:}\Op{\KWD{@}}\KWD{\boldmath{$>$}}}
  {\index{\~<\~:>@$\sim$$<$ $\sim$:$>$}
  Act like \kwd{pprint-logical-block} using \VAR{body} as \MC{format}
  ...
  \KWD{\TLD:@\boldmath{$>$}}, spaces in body are replaced with
...
\begin{LIST}{1cm}
  ...
  of
  \VAR{two-way-stream}/\VAR{echo-stream},
  respectively. 
  }
...
\begin{LIST}{1cm}
  ...
  \IT{(\FU*{PARSE-NAMESTRING} \VAR{foo} 
    \OP{\VAR{host}
      \OP{\VAR{default-pathname}\DF{\V{\A default-pathname-defaults\A}}
        \OP{\orGOO{\kwd{:start} \VAR{ start}\DF{\LIT{0}}\\
      \kwd{:end} \VAR{ end}\DF{\NIL}\\
