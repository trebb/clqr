...
\begin{LIST}{1cm}
  ...
  \IT{(\SF*{DEFSTRUCT}
    \GOO{\VAR{name}\XOR
      ...
      \orGOO{%
        ...
        \xorGOO{%
          \kwd{:constructor}\\
          (\kwd{:constructor} \Op{\VAR{make-name}\DF{\LIT{make-}\VAR{name}}
            \Op{({\OPn{\VAR{arg}}})}})}{\}^{\!\!*}}\\
        \xorGOO{%
          \kwd{:copier}\\
          (\kwd{:copier}
          \Op{\VAR{copy-name}\DF{\LIT{copy-}\VAR{name}}})}{\}}\\
        ...
        (\kwd{:type} \VAR{ type}) \Op{(\kwd{:initial-offset}
          \VAR{ n})}\\
        \kwd{:named}\\
        \xorGOO{%
          \kwd{:predicate}\\
  ...
  \IT{(\FU*{COPY-STRUCTURE} \VAR{struct})} 
  Return \retval{copy of structure} with shared slot values.

  \IT{(\FU*{SLOT-MAKUNBOUND} \VAR{instance} \VAR{slot})}
  Make \VAR{slot} \retval{\VAR{instance}} unbound.

  \IT{(\GFU*{SLOT-MISSING} \VAR{class} \VAR{object} \VAR{slot}
    \Goo{\kwd{setf} \kwd{slot-boundp} \kwd{slot-makunbound}} \Op{\VAR{value}})}
  Called in case of attempted access to missing \VAR{slot}. Default
  method signals an error.

  \IT{(\GFU*{SLOT-UNBOUND} \VAR{class} \VAR{instance} \VAR{slot}))}
  Called by \kwd{slot-value} in case of unbound \VAR{slot}. Default
