...
\begin{LIST}{1cm}
  ...
  as \VAR{test} on any set of elements of \VAR{sequence}s returns non-\NIL. 
  }
...
\begin{LIST}{1cm}
  ...
  {Return \retval{element of
    \VAR{sequence}} pointed to by zero-indexed \VAR{index}. \kwd{setf}able.
    }
  ...
  \IT{(\xorGOO{\FU*{SORT}\\
      \FU*{STABLE-SORT}}{\}} \VAR{sequence} \VAR{test}
    \Op{\kwd{:key} \VAR{function}})}
  {
  Return \retval{\VAR{sequence} sorted}. \VAR{sequence} is modified. Order
  ...
  \IT{(\xorGOO{\FU*{SUBSTITUTE}\\
      \FU*{NSUBSTITUTE}}{\}} \VAR{new}
    \VAR{old} \VAR{sequence} 
    \orGOO{\kwd{:from-end} \VAR{ bool}\DF{\NIL}\\
      \kwd{:test} \VAR{ function}\\
      \kwd{:test-not} \VAR{ function}\\
      \kwd{:start} \VAR{ start}\DF{0}\\
  ...
  {
  Make \retval{copy of \VAR{sequence}} with all (or \VAR{count})
  elements satisfying test replaced by
