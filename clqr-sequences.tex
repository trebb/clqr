...
\begin{LIST}{1cm}
  ...
  \IT{(\FU*{COUNT} \VAR{foo} \VAR{sequence} 
    \orGOO{\kwd{:from-end} \VAR{ bool}\DF{\NIL}\\
      ...
      \kwd{:key} \VAR{ function}}{\}})}
  {
  Return \retval{number of \VAR{foo}s} in \VAR{sequence} which satisfy
  tests. 
  }
  ...
  satisfies tests, or its \retval{position} relative to
  \VAR{sequence}'s start, respectively. 
  }
  ...
  \retval{its position} relative to \VAR{sequence}'s start, respectively.
  }
