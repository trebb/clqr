...
\begin{LIST}{1cm}
  ...
  \IT{\arrGOO{(\FU*{CONSP} \VAR{ foo})\\
      (\FU*{LISTP} \VAR{ foo})}{\}}}
  {
    Return \retval{\T} if \VAR{foo} is either \T\ or a \kwd{cons}/\kwd{list}.
    }
  ...
  \IT{(\xorGOO{\FU*{MEMBER}\\
      \FU*{MEMBER-IF }\VAR{predicate}\\
      \FU*{MEMBER-IF-NOT }\VAR{predicate}}{\}}
    \VAR{foo} \VAR{list}
...
\begin{LIST}{1cm}
  ...
  {
  Return new cons \retval{(\VAR{foo} \VAR{bar})}.
  }
  ...
  \IT{(\FU*{MAKE-LIST} \VAR{num} \Op{\kwd{:initial-element}
      \VAR{foo}\DF{\NIL}})}
  {Return \retval{list with \VAR{num} elements}
  set to \VAR{foo}.
  }
  ...
  \IT{(\xorGOO{\FU*{BUTLAST}\\\FU*{NBUTLAST}}{\}} \VAR{foo}
    \Op{\VAR{num}})}
  {
  ...
  {Return \VAR{n}th element (zero-based) of \VAR{list}.
    }

  \IT{(\kwd*{NTHCDR} \VAR{n list})}
  {Return \retval{tail of \VAR{list}} after calling \kwd{cdr} \VAR{n} times.
    }
  ...
  \IT{(\FU*{PUSHNEW} \VAR{foo} \VAR{place} )}
  {Set \VAR{place} to
  \retval{(\kwd{adjoin} \VAR{foo} \VAR{place})}.
  }

  \IT{\arrGOO{(\FU*{APPEND} \OPn{\VAR{ list}})\\
      (\FU*{NCONC}
      \OPn{\VAR{ list}})}{.}}
  {
