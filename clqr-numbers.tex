...
\begin{LIST}{1cm}
  ...
  \IT{(\FU*{INTEGER-LENGTH} \VAR{integer})} \retval{Number of bits} necessary to represent \VAR{integer}.

  \IT{(\FU*{BYTE} \VAR{size} \VAR{position})}
  \retval{Byte specifier} for a byte of \VAR{size} bits starting at a weight of $2^{\VAR{position}}$.
  ...
  Extract \retval{byte} denoted by \VAR{byte-spec} from \VAR{integer}. \kwd{setf}able.
  ...
  by corresponding bits of \VAR{int-a}, or by the low (\kwd{byte-size} \VAR{bytespec}) bits
  of \VAR{int-a}, resp.
