...
\begin{LIST}{1cm}
  ...
  \IT{\arrGOO{(\FU*{EVENP } \VAR{integer})\\
      (\FU*{ODDP } \VAR{integer})}{.}}
  {
    \retval{\T} if \VAR{integer} is even or odd, respectively.
  }
...
\begin{LIST}{1cm}
  ...
  \IT{(\FU*{RANDOM} \VAR{limit} \Op{\VAR{state}\DF{\V{\A random-state\A}}})} 
  {
...
\begin{LIST}{1cm}
  ...
  \begin{LIST}{.5cm}
    ...
    \IT{\CNS*{BOOLE-ORC1}\qquad\qquad} {\retval{$\lnot \text{\VAR{int-a}} \lor \text{\VAR{int-b}}$}.}
    \IT{\CNS*{BOOLE-ORC2}\qquad\qquad} {\retval{$\text{\VAR{int-a}} \lor \lnot\text{\VAR{int-b}}$}.}
