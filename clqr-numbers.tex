...
\begin{LIST}{1cm}
  ...
  \IT{(\xorGOO{\FU*{MOD}\\
      \FU*{REM}}{\}} \VAR{n} \Op{\VAR{d}\DF{\LIT{1}}})}
  {Same as \FU{floor} or
  \FU{truncate}, respectively, but return \retval{re\-main\-der} only.
  }

  \IT{(\FU*{RANDOM} \VAR{limit} \Op{\VAR{state}\DF{\V{\A random-state}}})} 
  {
...
\begin{LIST}{1cm}
  ...
  \begin{LIST}{.5cm}
    ...
    \IT{\CNS*{BOOLE-C1}} {\retval{$\lnot\text{\VAR{int-a}}$}.}
    \IT{\CNS*{BOOLE-C2}} {\retval{$\lnot\text{\VAR{int-b}}$}.}
    ...
    \IT{\CNS*{BOOLE-ORC1}} {\retval{$\lnot \text{\VAR{int-a}} \lor \text{\VAR{int-b}}$}.}
    \IT{\CNS*{BOOLE-ORC2}} {\retval{$\text{\VAR{int-a}} \lor \lnot\text{\VAR{int-b}}$}.}
